...
impl pubsub::Pubsub for PubSubConfluentKafka {
    ...
    fn pub_open(&mut self) -> Result<Self::Pub, Error> {
        let bootstap_servers = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_ENDPOINT",
            &self.host_state.config_toml_file_path,
        )?)?;
        let security_protocol = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_SECURITY_PROTOCOL",
            &self.host_state.config_toml_file_path,
        )?)?;
        let sasl_mechanisms = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_SASL_MECHANISMS",
            &self.host_state.config_toml_file_path,
        )?)?;
        let sasl_username = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_SASL_USERNAME",
            &self.host_state.config_toml_file_path,
        )?)?;

        let sasl_password = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_SASL_PASSWORD",
            &self.host_state.config_toml_file_path,
        )?)?;
    ...
    fn sub_open(&mut self) -> Result<Self::Sub, Error> {
        ...
        let group_id = String::from_utf8(runtime_configs::get(
            &self.host_state.secret_store,
            "CK_GROUP_ID",
            &self.host_state.config_toml_file_path,
        )?)?;
